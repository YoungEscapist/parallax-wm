...
void main() {
    ...
    // не покрашенной.
    if (glow_strength > 0.0 && win_rect.z > 0.0 && win_rect.w > 0.0) {
        ...
        // gd < 0 внутри. Полоса шириной glow_width вдоль края: у самого края
        // единица, глубже внутрь плавно в ноль.
        float rim = 1.0 - smoothstep(0.0, max(glow_width, 0.5), -gd);
        // Наружу (gd > 0) не светим вовсе: там уже не окно.
        rim *= step(gd, 0.0);
        color.rgb += glow_color * (rim * glow_strength) * color.a;
    }
...
impl Шейдер {
    pub fn new(renderer: &mut GlesRenderer) -> Option<Self> {
        let uniforms = [
            ...
            UniformName::new("glow_color", UniformType::_3f),
            UniformName::new("glow_width", UniformType::_1f),
            UniformName::new("glow_strength", UniformType::_1f),
        ];
    ...
    pub fn new(
        ...
        Self {
            ...
            uniforms: vec![
                ...
                Uniform::new("glow_color", свечение.цвет),
                Uniform::new("glow_width", свечение.ширина),
                Uniform::new("glow_strength", свечение.сила),
            ],
        }
    }
...
/// Кайма света по краю окна: чем светить, какой ширины полоса и насколько
/// сильно. Считается по палитре обоев и настройкам — см. [`Свечение::для_окна`].
...
impl Свечение {
    pub fn нет() -> Self {
        Self { цвет: [0.0, 0.0, 0.0], ширина: 0.0, сила: 0.0 }
    }
