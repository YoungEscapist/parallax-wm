...
fn собранные_фичи() -> Vec<&'static str> {
    ...
    // `shaders` появилась позже остальных и в этот список не попала: plx-extra
    // сообщал «features: vr, mine, share», хотя куб, свет и свечение окон в нём
    // собраны. Строчку из `--version` просят вставлять в отчёт об ошибке —
    // то есть врала она ровно там, где по ней судят о сборке.
