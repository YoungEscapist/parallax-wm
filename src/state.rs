...
/// Незаконченный выход из куба: куда уходим и что сделать, когда доиграет.
///
/// Раньше выход из куба был мгновенным — обзор гас в том же кадре, в котором
/// нажали Super, и куб просто ИСЧЕЗАЛ с экрана. Отдаление на входе без
/// приближения на выходе смотрится хуже, чем отсутствие обоих, поэтому
/// закрытие живёт отдельным состоянием, а не побочным эффектом клика.
#[derive(Clone, Copy, Debug)]
pub struct КубВыход {
    /// На какой стол приземляемся; `None` — остаёмся на текущем.
    pub стол: Option<u32>,
    /// Снять фокус и активность со всех окон в конце (тап Super хочет весь
    /// стол целиком, без выделенного окна; клик по грани — не хочет).
    pub сбросить_фокус: bool,
}
...
pub struct Parallax {
   ...
   pub pan_button_held: bool,
    ...
    /// Палитра ОБОЕВ — ею красится свечение окон (см. src/обои.rs и
    /// rounded.rs). `None`, пока plx-wall не посчитал палитру или не запущен
    /// вовсе; тогда свечение просто не включается.
    pub палитра_обоев: Option<crate::обои::Палитра>,
    /// Когда файл палитры правился в прошлый раз — по нему решаем, перечитывать
    /// ли. Читать его каждый кадр незачем: обои меняются раз в минуты.
    pub палитра_правка: Option<std::time::SystemTime>,
    /// Картинка заливки света на холсте (см. src/свет.rs). Строится один раз на
    /// смену обоев и живёт здесь, а не в кадре: пересобирать градиент 256×256
    /// каждый кадр незачем.
    pub свет: crate::свет::Свет,
    ...
    /// Сколько размера НАБРАЛА рука с начала жеста, в единицах холста. Целевой
    /// размер считается от снимка `gesture_resize_group` плюс это, а не
    /// накоплением от текущей геометрии: у накопления та же беда, что у щипка
    /// выше, — клиент применяет configure с задержкой, и подряд идущие события
    /// читают один и тот же старый размер.
    pub жест_размер_накоп: (f64, f64),
    /// Места окон на начало жеста ресайза. Нужны, когда тянут за ЛЕВЫЙ или
    /// ВЕРХНИЙ край: противоположный угол обязан стоять, значит окно едет на
    /// столько же, на сколько выросло, — и считать это надо от начального
    /// места, иначе сдвиг копится каждым событием (см. `размер_шагом_краем`).
    pub жест_размер_места: Vec<(Window, Point<i32, Logical>)>,
    ...
    /// Шейдер куба рабочих столов (см. src/куб.rs). Компилируется там же и
    /// живёт столько же, что и `blur_shape` выше.
    pub куб_шейдер: Option<crate::куб::Шейдер>,
    /// Текущий поворот куба, радианы. 0 — грань стола, стоящего первым в
    /// `overview_order`, смотрит прямо на зрителя.
    pub куб_угол: f64,
    /// Куда куб доворачивается. Разница с `куб_угол` — то, что доигрывает
    /// `anim::tick`; равенство означает «куб стоит».
    pub куб_цель: f64,
    /// Подложка грани: однотонная плитка, растягиваемая на всю грань. Без неё
    /// окна висели бы в пустоте и куба не читалось бы вовсе.
    pub куб_фон: Option<smithay::backend::renderer::element::memory::MemoryRenderBuffer>,
    /// Пока true, куб рисуется и ВНЕ обзора: идёт проворот на соседний стол
    /// (`set{ cube_switch = true }`). Гасится, когда поворот доигран.
    pub куб_переход: bool,
    /// Столы на гранях в порядке обхода — тот же список, по которому куб
    /// рисуется и считается «какой стол впереди». Снимается на входе в куб,
    /// чтобы грани не переставлялись под руками во время поворота.
    pub куб_столы: Vec<u32>,
    /// Зум куба: множитель к `cube_fill`, то есть к доле экрана, которую
    /// занимает передняя грань. Колесо в обзоре меняет ЕГО, а не зум камеры:
    /// камера на куб не смотрит вовсе (см. `куб_зум`).
    pub куб_масштаб: f64,
    /// Куда `куб_масштаб` едет. Разница с ним — то, что доигрывает `куб_тик`.
    ///
    /// Отдельное поле, а не мгновенная запись в `куб_масштаб`, потому что зум
    /// куба — это ДВЕ разные вещи одной ручкой: щелчок колеса (цель прыгает на
    /// шаг, масштаб догоняет — так зум перестал быть рывком) и вход-выход из
    /// обзора (цель 1.0 при масштабе «грань во весь экран» — это и есть то
    /// самое отдаление, которого не было вовсе: куб просто возникал готовым).
    pub куб_масштаб_цель: f64,
    /// Куб доигрывает ЗАКРЫТИЕ обзора: грань разъезжается обратно на весь
    /// экран, и только когда доехала — обзор гасится по-настоящему.
    ///
    /// Пока здесь `Some`, `overview_active` ещё true (иначе грани начали бы
    /// брать не ячейку сетки, а экран целиком — и картинка прыгнула бы на
    /// первом же кадре анимации), а настоящий выход делает `anim::tick`.
    pub куб_выход: Option<КубВыход>,
    /// Первая кнопка мышиного аккорда: нажата, клиенту НЕ отдана, ждём вторую
    /// (см. аккорды.rs).
    pub аккорд_ждёт: Option<crate::аккорды::ОтложеннаяКнопка>,
    /// Идущий мышиный аккорд.
    pub аккорд: Option<crate::аккорды::ИдётАккорд>,
    /// Рамка, которую тянут аккордом «обвести и запустить»: ГДЕ НАЧАЛИ, в
    /// координатах холста (рисуется в экранных — см. udev.rs).
    ///
    /// Второй угол здесь не хранится нарочно: он всегда равен текущему
    /// положению курсора, и хранить его копию значит завести вторую правду о
    /// том же. Копия, обновляемая только в обработчике движения, отставала от
    /// курсора при любом переносе мимо него (телепорт курсора, синтетический
    /// ввод харнесса) — на харнессе это выглядело как рамка 0×0 при уехавшей
    /// на километр стрелке.
    pub аккорд_рамка: Option<Point<f64, Logical>>,
    /// За какой угол тянут размер аккордом `2-3`. Выбирается в начале аккорда
    /// по четверти окна под курсором — ровно как у Super+ПКМ.
    pub аккорд_край: crate::grabs::resize_grab::ResizeEdge,
    /// Нажатие, которое НЕ РАССМАТРИВАЕТСЯ как начало мышиного аккорда.
    ///
    /// Взводится на время одного синтетического события: так уходит клиенту
    /// задержанное аккордами нажатие (`аккорд_отдать_клиенту`) и так же идёт
    /// кнопка от пальца (`сенсор.rs`). Без флага первое зациклилось бы —
    /// отданное нажатие снова попало бы в `аккорд_кнопка` и снова задержалось,
    /// а второе значило бы, что каждый тычок пальцем ждёт 250 мс несуществующей
    /// второй кнопки.
    pub аккорд_мимо: bool,
    /// Пальцы на сенсорном экране и то, чем они сейчас заняты (см. сенсор.rs).
    pub сенсор: crate::сенсор::Сенсор,
    /// Какие кнопки мыши зажаты ПРЯМО СЕЙЧАС, в нумерации `аккорды::Кнопка`.
    /// Нужно аккордам с прокруткой: «пока держат ЛКМ, колесо зумит».
    pub аккорд_зажаты: [bool; 3],
    /// Чьё НАЖАТИЕ аккорды съели. Отпускание такой кнопки клиенту тоже не
    /// отдаётся: он не видел нажатия, и голое отпускание правой кнопки
    /// открывало у него контекстное меню (см. `аккорд_кнопка`).
    pub аккорд_съедено: [bool; 3],
    /// Рамка выделения, начатая ЗАРАНЕЕ — под задержанной левой кнопкой, ещё до
    /// того, как стало ясно, аккорд это или обычный клик.
    ///
    /// `Some` — рамка идёт; внутри лежит выделение, которое было ДО неё, чтобы
    /// вернуть его, если из кнопки всё-таки вышел аккорд. Без раннего начала
    /// выделение появлялось только через `mouse_chord_timeout` (250 мс) — а
    /// человек к тому времени уже протащил мышь, и рамка выглядела «начинающейся
    /// с задержкой и не оттуда».
    pub аккорд_выделение: Option<Vec<Window>>,
    /// Обведённая рамка, ждущая своё окно: угол, размер и когда обвели.
    /// Забирает её первое же появившееся окно (`xwin::insert_new_window`).
    pub аккорд_запуск_в_рамке: Option<(Point<f64, Logical>, (f64, f64), std::time::Instant)>,
    /// Драг куба: экранный x начала жеста, поворот в этот момент и путь,
    /// пройденный курсором. Путь нужен, чтобы отличить протяжку (крутим) от
    /// клика (уходим на грань).
    pub куб_драг: Option<(f64, f64, f64)>,
    ...
    /// Пул под рамку мышиного аккорда «обвести и запустить». СВОЙ, а не общий
    /// со снимком: damage tracker ведёт историю по Id, и две разные плашки с
    /// одним номером в одном кадре затирали бы историю друг друга.
    pub аккорд_ids: Vec<crate::udev::SolidSlot>,
...
impl Parallax {
    ...
    pub fn new(event_loop: &mut EventLoop<'static, Self>, display: Display<Self>) -> Self {
        ...
        seat.add_pointer();
        // wl_touch объявляем только когда сенсор включён (`set{ touch }`).
        // Возможность места — не украшение: клиент, увидевший её, перестаёт
        // ждать эмуляцию мыши и начинает ждать касания. Выключенный сенсор,
        // объявивший `wl_touch`, оставил бы такое приложение без ввода вовсе.
        if lua_config.touch {
            seat.add_touch();
        }
        ...
        Self {
           ...
           pan_button_held: false,
            ...
            жест_размер_накоп: (0.0, 0.0),
            жест_размер_места: Vec::new(),
            needs_redraw: true,
            shot_request: None,
            ...
            аккорд: None,
            аккорд_рамка: None,
            аккорд_край: crate::grabs::resize_grab::ResizeEdge::BOTTOM_RIGHT,
            аккорд_мимо: false,
            сенсор: Default::default(),
            аккорд_зажаты: [false; 3],
            аккорд_съедено: [false; 3],
            аккорд_выделение: None,
            аккорд_запуск_в_рамке: None,
            куб_масштаб: 1.0,
            куб_масштаб_цель: 1.0,
            куб_выход: None,
            куб_драг: None,
            blur_ids: Vec::new(),
            portal_ids: Vec::new(),
    ...
    pub fn workspace_step(&mut self, dir: i32) {
        ...
        // Куб при переключении (Compiz): вместо мгновенной смены тегов экран
        // проворачивается на соседнюю грань. Набор граней снимаем ДО смены
        // тега — на нём стоит текущий стол, и с него поворот и начинается.
        self.куб_переход_начать(m);
