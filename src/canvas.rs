...
/// (см. `Parallax::minimap_hit`). Нераскрытая карточка не имеет права ни рисовать
...
/// `Parallax::minimap_window_at` ищет окно не по координате холста, а по ПРОЕКЦИИ —
...
mod tests {
    ...
    /// геометрии считается хит-тест (`Parallax::preview_contains`), поэтому «что
