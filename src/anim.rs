...
/// что и ТЕМП: ω спрашивают из мест, куда `&Dawn` не передан.
...
///
/// Атомик, а не поле в `Dawn`: длительность спрашивают из мест, которым `&Dawn`
...
impl ZoomGlide {
    ...
    /// `Dawn::завершить_анимации_камеры`). Тот же смысл, что у
    /// [`ZoomAnim::target`].
...
/// Постоянная времени ухода панели вверх (см. `Dawn::bar_hide`). Заметно
...
/// `Dawn::minimap_follow`). Заметно быстрее выезда панели: миникарта обязана
...
impl Dawn {
    ...
    pub fn anim_busy(&self) -> bool {
        self.camera_anim.is_some()
            ...
            || self.раздача.as_ref()
                .is_some_and(|раздача| раздача.гости.iter().any(|гость| гость.впущен))
            // Живые обои: тик обязан оставаться частым, пока dwall крутит
...
pub fn tick(state: &mut Dawn) {
    ...
    // а обоям нужен кадровый callback, иначе dwall засыпает навсегда (см.
