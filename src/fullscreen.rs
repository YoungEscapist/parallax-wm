...
//!
//! Экран у parallax — это окно в бесконечный холст: то, что видно, задаётся камерой
...
impl Parallax {
    ...
    /// уходила в никуда. Без callback'ов plx-wall засыпает, перестаёт читать
    ...
    /// запросы: plx-wall, промолчавший 300 мс, объявляет себя закрытым и
    /// перестаёт забирать кадры у декодера (см. plx-wall: «обои закрыты —
    /// засыпаем»). Callback'и рассылает хвост `render_surface`, то есть они
    /// идут ровно тогда, когда кадр собирается, — а кадр parallax собирает только
    /// по изменению на экране.
    ///
    /// На неподвижном экране это замыкается в круг: спящий plx-wall ничего не
    /// коммитит → рисовать нечего → callback'а нет → plx-wall спит дальше.
    ...
    ///
    /// Ответ на callback ещё не значит кадр: plx-wall в ответ нарисует свой
    ...
    /// plx-wall проспать дольше секунды — отметка `фон_коммит` протухает, сторож
    ...
    ///
    /// Спящему хватит одного callback'а: plx-wall на него нарисует кадр и
    ...
    pub fn будить_фоновые_слои(&mut self) {
        ...
        /// Пауза между побудками СПЯЩИХ обоев. Меньше порога plx-wall (300 мс,
    ...
    pub fn toggle_fullscreen(&mut self) {
        ...
        let Some(window) = self.window_to_fullscreen() else {
            tracing::debug!("plx/fullscreen: no window to maximise");
            return;
        };
    ...
    pub fn set_fullscreen(&mut self, window: &Window) {
        ...
        self.request_redraw();
        tracing::info!(
            "plx/fullscreen: fullscreen {}×{} requested (workspace {:#b}{}), waiting for the client frame",
            size.w, size.h, tags,
            if свой_стол { "" } else { ", not the current one" },
        );
    }
    ...
    ///
    /// Полный экран в parallax держится на договоре «окно стоит в той точке холста,
    ...
    pub fn resync_fullscreen_frame(&mut self) {
        ...
        //
        // Так выглядит слайд перелистывания столов (`Parallax::слайд_столов`):
        ...
        let причина = match текущее {
            None => "the window is not in space",
            Some(_) if !место_сходится => "the window is in the wrong place",
            _ => "the camera is in the wrong place",
        };
        ...
        if пора {
            ...
            tracing::debug!(
                "plx/fullscreen: возврат кадра к развёрнутому окну в ({},{}): {} \
    ...
    fn finish_fullscreen(&mut self, window: &Window, size: Size<i32, Logical>) {
        ...
        self.request_redraw();
        tracing::info!(
            "plx/fullscreen: window fullscreen {}×{} at ({},{}){}{}",
            size.w, size.h, loc.x, loc.y,
            if готов { "" } else { " (the client was too slow, switched on timeout)" },
            if свой_стол { "" } else { " (not the current workspace — geometry only)" },
        );
    }
    ...
    pub fn unset_fullscreen_window(&mut self, window: &Window) {
        ...
        self.arrange();
        self.request_redraw();
        tracing::info!("plx/fullscreen: leaving fullscreen");
    }
