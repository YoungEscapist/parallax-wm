...
//! `crates/halley-core/src/cluster.rs`. Там это называется кластером; в parallax у
...
//! него добираются через полоску кластера в его интерфейсе. У parallax такой
...
//!
//! Ещё не взяты режимы `Collapsed`/`Active` с узлом-ядром: в parallax у грозди
...
/// Зазор между членами грозди. У halley это `inner_gap` из настроек; в parallax
...
/// логических пикселях вместо f32: у parallax размеры окон целые, и считать их в
