    ...
    pub fn sync_x11_geometry(&mut self) {
        ...
        let updates: Vec<(X11Surface, Rectangle<i32, Logical>)> = self
            ...
            .filter_map(|window| {
                ...
                // Зажимать в [0, screen_size] верно только для одного монитора,
                // у которого дом (0,0) совпадает с его же местом в раскладке.
                // Прошлая правка (26.08.2026) поменяла границы на [дом,
                // дом+размер] — но `дом` это угол ВНУТРЕННЕГО прямоугольника
                // холста монитора, разнесённого на ШАГ_ДОМА (1 000 000) ровно
                // затем, чтобы столы двух мониторов НИКОГДА не пересекались на
                // холсте; к тому, что Xwayland считает своим RandR-экраном,
                // это отношения не имеет. RandR-раскладка Xwayland зеркалит
                // РЕАЛЬНОЕ место монитора — `Монитор::раскладка` (то самое
                // `monitor{ x=, y= }`), которое почти всегда МАЛЕНЬКОЕ число
                // (второй монитор обычно рядом или под первым). Отправляя
                // X-клиенту координату у монитора 2 около (1 000 020, 0),
                // получали ровно то, от чего чинились: Xwayland зажимал
                // указатель к границе СВОЕГО настоящего (маленького) экрана,
                // и клик утекал в чужой монитор — «столы смешиваются» была
                // именно эта путаница дом↔раскладка.
                //
                // Правильно: взять положение окна ОТНОСИТЕЛЬНО дома его
                // монитора (это и переживает зум/пан камеры), зажать этой
                // разностью в размер монитора, и уже эту относительную
                // точку прибавить к раскладке — реальному месту, которое
                // видит Xwayland.
                let монитор = self
                    .tagged_windows
                    .iter()
                    .find(|tw| &tw.window == window)
                    .and_then(|tw| self.монитор_стола(tw.tags))
                    .and_then(|i| self.мониторы.get(i));
                let (дом, размер, раскладка) = монитор
                    .map(|m| (m.дом, m.размер, m.раскладка))
                    .unwrap_or(((0, 0).into(), экран, (0, 0).into()));
                // Полноэкранное окно обязано стоять В РОВНО ТОЙ точке, что видит
                // Xwayland (раскладка), без клампа по `loc`. `loc` берётся из
                // camera/space и может на кадр-другой разойтись с домом монитора
                // (resync камеры в resync_fullscreen_frame, зум ≠ 1 — см.
                ...
                let loc = if self.is_fullscreen(window) {
                    раскладка
                } else {
                    let rel_x = (loc.x - дом.x).clamp(0, (размер.w - current.size.w).max(0));
                    let rel_y = (loc.y - дом.y).clamp(0, (размер.h - current.size.h).max(0));
                    smithay::utils::Point::from((раскладка.x + rel_x, раскладка.y + rel_y))
                };
