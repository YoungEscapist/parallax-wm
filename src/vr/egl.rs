//! Привязка OpenXR к тому самому EGL-контексту, в котором dawn уже рисует.
//!
...
//! `OpenGL` требует настольный GLX/WGL-контекст. У dawn же весь рендер идёт
...
//! рисовать в них тем же кодом, что и всё остальное в dawn. Никакого
...
//! линковаться с libEGL напрямую dawn не хочет: EGL приходит с драйвером и
...
/// То, что рантайм считает требованиями к версии GL ES. Держим как есть: dawn
