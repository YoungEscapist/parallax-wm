...
//! вещи снаружи dawn: должен идти `wivrn-server`, загрузчик OpenXR должен найти
//! манифест WiVRn, и человек должен надеть шлем и запустить на нём клиент.
//! Первые две dawn берёт на себя здесь, третью — ждёт (см. `vr::Ожидание`).
//!
...
//! Здесь всё то же самое, но без root и без скрипта — dawn уже идёт от нужного
...
/// любого, у кого это слово мелькнуло в аргументах, — на этом в dawn уже
/// обжигались не раз.
...
///
/// **Парная функция обязательна.** Окружение dawn наследуют все запускаемые из
...
pub fn подставить_рантайм() {
    ...
    tracing::info!("dawn/vr: беру рантайм {}", манифест.display());
...
///
/// `setsid` — чтобы сервер пережил перезапуск dawn по Super+R: шлем при этом
/// не отваливается, а новый dawn находит уже поднятый сервер.
