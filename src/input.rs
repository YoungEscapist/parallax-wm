...
impl Parallax {
    /// Отдать клиенту кнопку, которую аккорды придержали и не использовали.
    ///
    /// Живёт здесь, а не в `аккорды.rs`, потому что это единственное место, где
    /// у нас есть указатель seat'а и договорённость «после `button` обязателен
    ...
    /// То же выделение, но БЕЗ серийника события: его берём свой.
    ///
    /// Нужно аккордам: рамку они начинают не на пришедшем нажатии (оно
    /// задержано и своего серийника в этот момент не тратит), а сразу, чтобы
    /// выделение не отставало от руки на `mouse_chord_timeout`.
    pub(crate) fn начать_выделение_сейчас(&mut self) -> bool {
    ...
    pub fn process_input_event<I: InputBackend>(&mut self, event: InputEvent<I>) {
        match event {
            ...
            InputEvent::PointerMotion { event, .. } => {
               ...
               // Мышиный аккорд ведёт окно, размер, рамку или камеру — раньше
               // всех прочих драгов по той же причине, по которой ниже раньше
               // всех стоит драг куба: аккорд уже держит две кнопки, и любая
               // ветка, решающая «а не пан ли это», решала бы про тот же жест
               // второй раз.
               if self.аккорд_движение(delta.x, delta.y, event.time_msec()) {
                   return;
               }
               ...
               // Драг куба — раньше пана холста и раньше всего прочего: пока
               // куб на экране, холста под ним не видно, и таскать камеру
               // бессмысленно.
               //
               // Стрелка при этом ЕДЕТ вместе с рукой (та же правка, что у пана
               // карточки предпросмотра, — жалоба «курсор остаётся на месте и
               // статичен»): на отпускании по ней считается грань, на которую
               // мы уходим, и примёрзшая стрелка означала бы «отпустил над
               // одной гранью, попал на другую».
               if self.куб_драг.is_some() {
                   self.куб_драг_движение(delta.x);
            ...
            InputEvent::PointerButton { event, .. } => {
                ...
                // МЫШИНЫЕ АККОРДЫ (hevel) — первыми и по тому же договору, что
                // таблица жестов: вернули `true` — событие наше целиком, ни
                // прежним веткам, ни клиенту его не видать. Пустая таблица
                // отвечает `false` на всё, то есть мышь работает как раньше.
                //
                // После счётчика кнопок, а не до: аккорд глотает нажатие, но
                // кнопка физически нажата, и счётчик обязан это знать — иначе
                // переход курсора на соседний монитор разрешался бы посреди
                // протяжки.
                if self.аккорд_кнопка(button, ButtonState::Pressed == btn_state, event.time_msec()) {
                    return;
                }
                ...
                if self.overview_active {
                    // ── КУБ: ЛКМ его крутит, а короткий клик уводит на грань ──
                    //
                    // Основному хендлеру событие не отдаём вовсе: окна на кубе
                    // нарисованы не там, где лежат на холсте, и `element_under`
                    // ответил бы про окно из совсем другого места экрана —
                    // клик по грани уносил на чужой стол.
                    if self.куб_активен() && button == BTN_LEFT && !alt_held && !self.logo_held {
                        if ButtonState::Pressed == btn_state {
                            self.куб_драг_начать();
                        } else if self.куб_драг_кончить() {
                            let цель = self
                                .куб_стол_в_точке(self.pointer_screen_physical())
                                .or_else(|| self.куб_передний_стол());
                            self.exit_overview_immediate(
                                цель.filter(|&m| m != self.viewport.current_tags()),
                            );
                        }
                        return;
                    }
            ...
            InputEvent::PointerAxis { event, .. } => {
                ...
                if source == AxisSource::Finger {
                    // Всё, что ниже, — только про СВОЙ жест ветки оси. Идущий
                    // жест libinput (три-четыре пальца) отсюда не трогаем ни
                    // концом, ни кадрами: пальцы на тачпаде одни, и прокрутка,
                    // прилетевшая посреди трёхпальцевого свайпа, — это его
                    // же пальцы, а не новый двухпальцевый жест.
                    // Сторож зовётся и ЗДЕСЬ, а не только в `anim::tick`: тик
                    // идёт, пока есть что перерисовывать, а зависший жест
                    // ничего не рисует — на неподвижном экране до него дошли
                    // бы только вместе со следующей анимацией.
                    self.жест_сторож();
                    let чужой = self.жест.is_some() && !self.жест_из_оси();
                    if чужой {
                        // Пусть доигрывает свой обработчик.
                    } else if h == 0.0 && v == 0.0 {
                        ...
                        if self.жест.is_none()
                            && self.жест_начало_из(crate::gestures::ОсноваЖеста::Свайп, 2, true)
                        {
                ...
                // ── КУБ: колесо ЗУМИТ, поворот — по горизонтали и драгом ──
                //
                // Зум здесь не камеры (она на куб не смотрит вовсе, кадр задаёт
                // build_cube_elements), а самого куба: колесо меняет долю
                // экрана под передней гранью. Раньше колесо крутило грани, и
                // отодвинуть куб было нечем — жалоба «куб не даёт зумить».
                // Поворот остался у горизонтального колеса, Shift+колеса и у
                // драга ЛКМ (см. куб_драг_движение).
                if self.куб_активен() && source != AxisSource::Finger {
                    let shift = self.seat.get_keyboard()
                        .map(|kb| kb.modifier_state().shift)
                        .unwrap_or(false);
                    if h != 0.0 || (shift && v != 0.0) {
                        let d = if h != 0.0 { h } else { v };
                        // Знак: колесо ВНИЗ (d > 0) — на СЛЕДУЮЩИЙ стол, как
                        // Super+PgDn. В `куб_повернуть` следующая грань стоит
                        // на минус шаг (угол грани i равен −i·шаг, см.
                        // куб_угол_стола), поэтому знак здесь обратный.
                        self.куб_повернуть(if d < 0.0 { 1.0 } else { -1.0 });
                        return;
                    }
                    if v != 0.0 {
                        self.куб_зум(if v < 0.0 { 1.1 } else { 1.0 / 1.1 });
                        return;
                    }
                }
