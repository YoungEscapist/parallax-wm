...
pub fn focus(state: &mut Parallax, window: &Window) {
    ...
    // (на мониторе это мигание рабочим столом) и забирает у игры клавиатуру —
    // то есть терминал, открытый Super+Enter прямо из игры, выкидывает игрока
    // из неё.
    //
    // Проверка стоит ИМЕННО ЗДЕСЬ, а не у одного вызывающего. `focus()` — общая
    // воронка: сюда сходятся sloppy focus от движения мыши (input.rs), клик,
    // Alt+Tab (switcher.rs), обзор, лента колонок, чип на панели, закрытие
    // соседа, появление нового окна — тринадцать мест. Правка 01.09.2026
    // перенесла ветку отсюда в `insert_new_window`, и все остальные двенадцать
    // снова начали поднимать панель поверх игры на кадр-другой: жалоба «новые
    // окна всё ещё на долю секунды перекрывают игру» — это они.
    //
...
impl Parallax {
    ...
    pub fn insert_new_window(&mut self, window: Window, size: Size<i32, Logical>, floating: bool) {
        ...
        // Автоматически отдаём фокус новому окну (как sway/hyprland). В режиме
        // Minecraft `focus()` сам отдаёт его месту мода, не трогая ни верх
        // стопки, ни хозяйскую клавиатуру, — своей ветки здесь не нужно.
        focus(self, &window);
