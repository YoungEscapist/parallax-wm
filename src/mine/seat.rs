...
//!
//! **Почему не общий ввод хозяина, как было.** Игра — такое же окно dawn, и
...
//! получают ввод от места `dmine`. Тот же приём, что у гостей `dshare`
...
pub fn завести(state: &mut Dawn) -> Option<Seat<Dawn>> {
    let dh = state.display_handle.clone();
    let mut место = state.seat_state.new_wl_seat(&dh, "dmine".to_string());
    // Раскладка та же, что у хозяина: клавиши приходят скан-кодами evdev, а
    // раскладку к ним применяет dawn.
    let раскладка = state.lua_config.xkb_config();
    if место.add_keyboard(раскладка, 200, 25).is_err() {
        tracing::warn!("dawn/mine: место без клавиатуры — отказ");
        return None;
    }
    место.add_pointer();
    tracing::info!("dawn/mine: у мода своё место");
    Some(место)
}
...
fn место(state: &Dawn) -> Option<Seat<Dawn>> {
...
/// клику хозяина.
pub fn кнопка(state: &mut Dawn, окно: Option<Window>, код: u32, нажата: bool) {
    let Some(место) = место(state) else { return };
    let serial = SERIAL_COUNTER.next_serial();
    let время = state.start_time.elapsed().as_millis() as u32;
    if нажата {
...
pub fn колесо(state: &mut Dawn, dx: f64, dy: f64) {
...
/// Из игры они не работали — при том, что в самом dawn работают.
///
...
pub fn клавиша(state: &mut Dawn, код: u32, нажата: bool) {
    let Some(место) = место(state) else { return };
