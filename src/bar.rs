...
//!
//! Почему острова, а не одна полоса. parallax рисует интерфейс скруглёнными
...
//! и разбор кликов (`Parallax::bar_click`) зовут одну и ту же [`layout`] — второй
//! копии чисел нет намеренно. Ровно так устроена полка (`tray::layout`), и
//! ровно на разъехавшихся копиях геометрии в parallax уже однажды поехали клики по
...
///
/// Раньше чип был БУКВОЙ, и в коде тут стояло «значка приложения у parallax взять
...
/// `Parallax::bar_data`) и идёт и в отрисовку, и в хит-тест.
...
pub fn share_text(код: &str, гостей: usize) -> String {
    if гостей > 0 {
        тф!("код {код} · {гостей}", "code {код} · {гостей}")
    } else {
        тф!("код {код}", "code {код}")
    }
}
...
pub fn layout(d: &Data) -> Layout {
    ...
    // из-за чего в parallax однажды разъехались клики по окнам.
    let _ = влезает;
...
pub fn date_text() -> String {
    // Порядок от воскресенья: `tm_wday` в struct tm считается именно так.
    const ДНИ_RU: [&str; 7] = ["Вс", "Пн", "Вт", "Ср", "Чт", "Пт", "Сб"];
    const ДНИ_EN: [&str; 7] = ["Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"];
    let дни = т!(ДНИ_RU, ДНИ_EN);
    match local_time() {
        Some(tm) => {
...
impl crate::state::Parallax {
    ...
    pub fn bar_data(&self) -> Data {
        Data {
            ...
            share: self.раздача_код_и_гостей(),
        }
    }
        ...
        let найденный = self.icon_cache
            ...
            .or_else(|| match window.underlying_surface() {
                smithay::desktop::WindowSurface::X11(s) => {
                    ...
                    tracing::debug!(
                        "plx/icons: icon \"{}\" from _NET_WM_ICON — {}",
                        app_id, if значок.is_some() { "found" } else { "missing" },
                    );
                    значок
                }
                _ => None,
    ...
    pub fn bar_hover_update(&mut self, pos: smithay::utils::Point<f64, smithay::utils::Physical>) {
        ...
        // ней панят и зумят, как по карте, см. `Parallax::preview_*`). Считать это
...
mod tests {
    ...
    ///
    /// Язык ставится ЯВНО и проверяются оба: подпись считает ширину острова
    /// (см. `share_text`), и молчаливая смена умолчания языка увела бы
    /// геометрию панели, а тест бы этого не заметил.
    #[test]
    fn подпись_раздачи_считает_гостей() {
        use crate::lang::{установить, текущий, Язык};
        let было = текущий();
        ...
        установить(Язык::En);
        assert_eq!(share_text("123456", 0), "code 123456");
        assert_eq!(share_text("123456", 2), "code 123456 · 2");
    ...
    fn часы_и_дата_нужной_формы() {
        ...
        // Проверяем ФОРМУ, а не длину: сокращения дня разной длины («Пн» — две
        // буквы, «Mon» — три), и общего числа символов у этих строк нет.
        // Хвост же одинаков всегда: пробел и «дд.мм».
        for язык in [Язык::Ru, Язык::En] {
            установить(язык);
            let date = date_text();
            let символы: Vec<char> = date.chars().collect();
            let хвост: String = символы[символы.len() - 6..].iter().collect();
            assert_eq!(хвост.chars().next(), Some(' '), "перед датой нет пробела: {date:?} ({язык:?})");
            assert_eq!(хвост.chars().nth(3), Some('.'), "дата не «дд.мм»: {date:?} ({язык:?})");
            assert!(
                хвост[1..].chars().filter(|c| *c != '.').all(|c| c.is_ascii_digit()),
                "в дате не только цифры: {date:?} ({язык:?})"
            );
            let день: String = символы[..символы.len() - 6].iter().collect();
            assert!(
                (2..=3).contains(&день.chars().count()) && день.chars().all(|c| c.is_alphabetic()),
                "день недели не «Пн»/«Mon»: {день:?} ({язык:?})"
            );
        }
