...
--
-- ВТОРОЙ ПОЛОВИНОЙ МОЖЕТ БЫТЬ ПРОКРУТКА: "1-scroll" — это «пока держу левую,
-- колесо зумит», ровно как Alt+колесо, только вместо Alt рука на кнопке.
-- Такая пара НЕ ЗАДЕРЖИВАЕТ ничего: задержка нужна лишь затем, чтобы не отдать
-- клиенту нажатие, которое окажется началом аккорда, а прокрутка приходит
-- сама. Поэтому "1-scroll" не отнимает у левой кнопки ни мгновенности клика,
-- ни рамки выделения по пустому холсту, — в отличие от "1-2" (левая, затем
-- НАЖАТИЕ на колесо). Прокруткой ведутся только мгновенные действия: зум и
-- любое действие из общей таблицы; пан, ведение и размер требуют хода руки,
-- которого у колеса нет, и такие пары пропускаются с предупреждением в логе.
--
...
-- mouse{ chord = "2-3", action = "resize_window" }  -- тянет ТОТ УГОЛ, где стоит курсор
--
-- Размер тянется за угол окна, в четверти которого стоит курсор, — ровно как у
-- Super+ПКМ: в левой верхней четверти за рукой идут левый и верхний края, а
-- противоположный угол стоит на месте. Раньше размер просто прибавлялся к
-- правому нижнему углу, и край можно было только оттолкнуть, но не притянуть.
--
-- ДВОЙНОЙ КЛИК. Одна и та же кнопка дважды ("3-3") — тоже аккорд. Щёлкнул
-- дважды — окно поехало за курсором, щёлкнул ещё раз — положил. Потянул, не
-- отпуская второго щелчка, — работает как обычный драг и кончается
-- отпусканием.
-- mouse{ chord = "3-3", action = "move_window" }
-- Сколько ждать второй щелчок, считая ОТ ОТПУСКАНИЯ первого. Цена та же, что у
-- аккордов, только больше: пока на кнопку настроен двойной клик, её первый
-- щелчок ЦЕЛИКОМ — и нажатие, и отпускание — доходит до приложения на столько
-- позже.
-- set{ mouse_double_click = 400 }
-- mouse{ chord = "1-scroll", action = "zoom" }      -- ЛКМ + колесо = зум холста (Alt не нужен)
-- mouse{ chord = "1-2", action = "zoom" }           -- ЛКМ, затем нажать колесо: тянуть вверх-вниз = масштаб
-- mouse{ chord = "1-2", action = "toggle_fullscreen" }
-- set{ mouse_chord_timeout = 250 }
-- Во сколько раз ресайз аккордом сильнее хода руки (только мышь; у тачпада
-- своё усиление). Протяжка сперва делится на зум, поэтому на отдалённой
-- камере ощущается ровно так же, как на близкой.
-- set{ mouse_chord_resize_gain = 4.0 }

-- ── ТАЧСКРИН ────────────────────────────────────────────────────────────────
--
-- Сенсорный экран ноутбука. Включён по умолчанию: без него палец не делает
-- ровно ничего — parallax раньше не объявлял wl_touch вовсе.
--
-- Хозяин касания определяется тем, куда положили палец:
--
--   * НА ОКНО — касание уходит КЛИЕНТУ, настоящим wl_touch, со слотами и
--     мультитачем: браузер получает свою инерционную прокрутку, читалка —
--     щипок по странице. Тап заодно переводит фокус, как и клик.
--   * НА ОРГАН КОМПОЗИТОРА (панель, полка, карта окон, карточка предпросмотра,
--     обзор, куб, выбор источника для демонстрации) — палец работает
--     указателем: курсор переносится под него, дальше всё как с мышью.
--   * НА ПУСТОЙ ХОЛСТ — палец ведёт КАМЕРУ. Один панит (с той же инерцией, что
--     и свайп тачпада), два зумят щипком, три и больше идут в таблицу
--     gesture{} выше, общую с тачпадом. Тычок без протяжки просто снимает
--     фокус, как щелчок по пустому месту.
--
-- Курсор ходит за пальцем во всех трёх случаях: половина parallax считает
-- «где человек» по положению указателя.
--
-- Щипок ПО ОКНУ зумит клиента, а не холст: содержимое окна — его собственное
-- дело. Чтобы зумить холст откуда угодно, щипайте ТРЕМЯ пальцами (см. таблицу
-- gesture{}). В раскладках Tile и Columns пальцы камеру не двигают вовсе: там
-- вид принадлежит раскладке, и это то же правило, по которому живёт тачпад.
--
-- set{ touch = true }
-- set{ touch_pan = true }       -- палец по пустому холсту ведёт камеру
-- set{ touch_zoom = true }      -- два пальца щипком зумят холст
-- set{ touch_gestures = true }  -- три пальца и больше → таблица gesture{}
--
-- Палец, пролежавший неподвижно столько миллисекунд, становится ПРАВОЙ
-- кнопкой (контекстные меню — второй кнопки у пальца нет). 0 — выключить.
-- set{ touch_long_press = 500 }
-- Сколько пикселей экрана касание может проехать и всё ещё считаться тычком.
-- Тем же порогом меряется неподвижность для долгого нажатия выше.
-- set{ touch_tap_slop = 12 }
-- Отдавать касания НЕ клиентам, а эмуляции мыши. Для приложений, которые
-- wl_touch не поддерживают вовсе (старые игры под Xwayland, java-клиенты):
-- без этого они на палец не отзываются никак.
