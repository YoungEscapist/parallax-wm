#!/bin/bash
# Поднять headless-харнесс parallax: свой HOME, свой XDG_RUNTIME_DIR, своя шина
...
#     HOME=/tmp/parallax-harness/home XDG_RUNTIME_DIR=/tmp/parallax-harness/run \
...
set -u
WLDEBUG=${WLDEBUG:-}
root=/tmp/parallax-harness
# Харнесс проверяет ПОЛНУЮ сборку: иначе выключённые модули не отсматривались бы
# вовсе. Стандартную проверять тем же харнессом можно — подменив путь на plx-standard.
# Путь — `extra/release`, куда кладёт build.sh (см. тот же разбор в
...
# `setsid runuser`, записанный pid — обёртка, а `pkill -f parallax` рискует поймать
...
# XDG_RUNTIME_DIR — иначе parallax не найдёт его IPC-сокет. Под этот перебор он
...
if [ ! -f "$root/home/.config/parallax/config.lua" ]; then
    mkdir -p "$root/home/.config"
    cp -r "/home/$user/.config/parallax" "$root/home/.config/" 2>/dev/null
    cp -r "/home/$user/.config/plx-wall" "$root/home/.config/" 2>/dev/null
...
# PLX_HEADLESS_SERVICES включать нельзя: полка держит имя
# org.kde.StatusNotifierWatcher и на общей шине отняла бы трей у живого parallax.
...
# `env -i` вычищает и LANG, а без него TUI-программы отказываются стартовать
# вовсе: btop пишет «No UTF-8 locale detected» и выходит с кодом 1, и в кадре
# вместо окна оказывается сообщение ghostty об ошибке. Переопределяется
# переменной HLANG (не LANG: та у самого харнесса своя и в env -i не идёт).
setsid runuser -u "$user" -- env -i \
  ...
  LANG="${HLANG:-C.UTF-8}" \
  PLX_HEADLESS_MODE="$mode" \
  ${names:+PLX_HEADLESS_NAMES="$names"} \
  PLX_HEADLESS_SERVICES=1 \
  ${WLDEBUG:+WAYLAND_DEBUG=1} \
  TZ=Europe/Riga \
  ${XR_RUNTIME_JSON:+XR_RUNTIME_JSON="$XR_RUNTIME_JSON"} \
  RUST_LOG=parallax=debug,info \
  dbus-run-session -- "$bin" --headless \
  > "$root/parallax.log" 2>&1 &

sleep 4
grep -a "parallax socket\|parallax/ctl" "$root/parallax.log" | tail -2
echo "лог: $root/parallax.log, снимки: $root/shots"
echo "обои: ./ctl.py 'action spawn cmd=\"/home/$user/.local/bin/plx-wall\"'"
